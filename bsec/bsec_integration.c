...
sleep_fct sleep;
get_timestamp_us_fct get_timestamp_us;
output_ready_fct output_ready;
state_save_fct state_save;

static uint32_t save_intvl;
...
{
    sleep = sleep_b;
    get_timestamp_us = get_timestamp_us_b;
...
void bsec_iot_loop()
{
    ...
    if (time_stamp_interval_ms > 0)
    {
        ...
        err_code = app_timer_start(m_bsec_timer_tick_src_id,
                                   APP_TIMER_TICKS((uint32_t)time_stamp_interval_ms),
                                   NULL);
        APP_ERROR_CHECK(err_code);
    }
